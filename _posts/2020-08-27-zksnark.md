...
encrypted secrets that are required by the cryptographic
...
First, let's define some parameters. We can use `m` to denote the
number of variables in a computation, `l` of which are
public. Take `x³ + x + 5 = 35` as example where the prover wants
...
We can express the 2nd constraint in bellman like this:
...
function ensures that this sequence of tuples is stored correctly for 
every variable.
...
with KeypairAssembly (basically executes all the `alloc`,
`alloc_input` and `enforce` in the circuit), what we end up with is
...
The last value we want to create during the trusted setup is `Lᵢ(τ)/δ`
where `Lᵢ(x) = β * Aᵢ(x) + α * Bᵢ(x) + Cᵢ(x)` as we discussed
...
is implemented in Bellman. It is fascinating how clever use of
math can potentially be part of the solution for our privacy issues in
the future. Many other ZKP systems has been developed in the past few
years and as more people become interested in this area, hopefully we
can provide a technical solution to the privacy/convenience dilemma,
instead of just relying on government regulations and the goodwill of big
internet companies.
