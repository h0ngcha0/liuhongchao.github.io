...
definitely one of the the most exciting cryptographic inventions after
[Public-key
...
convince third parties (the verifiers) that the prover faithfully executed
a program with some secret information, without conveying any
...
be used to outsource expensive computation.
...
This article assumes that the reader understands the QAP
transformation at a higher level, specifically why proving
`A(τ)*B(τ)-C(τ) = H(τ)*Z(τ)` is important in terms of completenss and
soundness. It walks through how zk-SNARK is implemented in
[Bellman](https://github.com/zkcrypto/bellman) (0.7.0), a
rust library used by the first widespread zk-SNARK application:
[ZCash](https://z.cash/). Bellman currently implements the
[Groth16](https://eprint.iacr.org/2016/260.pdf) proving system, which
can roughly be divided into three parts: **Trusted setup**, **Prover**
